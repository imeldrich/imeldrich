...
<section class="title">
    ...
    ?>
    <?php
        for($i = 0; $i < $fetch_cart['quantity']; $i++){
    ?>
    <div  class="box">
        ...
        <form action="" method="post">
            ...
            <select name="size" class="size" id="size" placeholder="<?php echo $fetch_cart['size']; ?>">
                    <option selected><?php echo $fetch_cart['size']; ?></option>
            ...
            <input type="number" min="1" max="10" oninput="quantity(this)" value="1" name="cart_quantity" class="qty">
            <label><?php echo $fetch_products['stock']; ?> :STOCK</label>
            <input type="submit" value="update" class="option-btn" name="update_quantity">
        </form>
        <div class="sub-total"> total : <span>₱<?php echo $sub_total = ($fetch_cart['price']); ?></span> </div>
    </div>
    <?php
        }
    ?>
